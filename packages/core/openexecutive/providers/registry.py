...
# Derived, not hand-listed. A literal map keyed on model ids rots the moment
# the ids are renamed — which is exactly what happened when this branch merged
# upstream: the trio became claude-opus-5 / claude-sonnet-5 / claude-haiku-4-5
# and all three hardcoded keys stopped matching, so every Claude model fell
# through `AgentSDKProvider._resolve_model`'s raw-id fallback and reached the
# CLI unmapped. The family segment of a Claude id IS the CLI's tier alias, so
# deriving it (the same move `openrouter_slug_for_claude` makes above) keeps
# this correct across renames. Previous-generation ids are included because an
# agent_overrides row persisted before a rename must still resolve.
_LEGACY_CLAUDE_IDS = ("claude-opus-4-7", "claude-sonnet-4-6", "claude-haiku-4-5-20251001")

_CLAUDE_CLI_MODELS: dict[str, str] = {
    model: m.group("family")
    for model in (*ANTHROPIC_DIRECT_MODELS, *_LEGACY_CLAUDE_IDS)
    if (m := _CLAUDE_ID_RE.match(model)) is not None
}
